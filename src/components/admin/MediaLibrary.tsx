...
export const MediaLibrary = component$<MediaLibraryProps>((props) => {
  ...
  // Actions would be used if implementing upload/delete within this component
  // const uploadAction = useUploadMedia();
  ...
  const loadMedia = $(async () => {
    ...
    try {
      ...
      if (getMediaAction.value?.success) {
        console.log('MediaLibrary: Media loaded successfully, count:', getMediaAction.value.data?.length);
        mediaStore.items = getMediaAction.value.data || [];
      } else {
        console.error('MediaLibrary: Media load failed:', getMediaAction.value?.message);
        mediaStore.error = getMediaAction.value?.message || 'Failed to load media';
      }
    } catch (error) {
      console.error('MediaLibrary: Load error:', error);
      mediaStore.error = error instanceof Error ? error.message : 'Failed to load media';
    } finally {
      mediaStore.loading = false;
    }
  });

  // Load media when component mounts or when modal opens
  useTask$(async ({ track }) => {
    track(() => isOpen.value);
  ...
  const toggleMediaSelection = $((media: MediaItem) => {
    const index = mediaStore.selectedItems.findIndex(item => item.id === media.id);
  ...
  const filteredMedia = mediaStore.items.filter(item =>
    !searchQuery.value ||
    item.filename.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
    item.meta?.description?.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div class="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div class="flex items-center justify-between p-4 border-b bg-gray-50">
              ...
              {/* Upload Button */}
              {allowUpload && (
                <label for="media-upload" class="btn btn-primary btn-sm">
                  <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {mediaStore.loading ? (
                ...
                <div class="text-center py-8 text-gray-500">
                  ...
                  <p class="text-xs mt-2">
                    Total items: {mediaStore.items.length} |
                    Search: "{searchQuery.value}" |
                    Action available: {getMediaAction ? 'Yes' : 'No'}
                  </p>
                  <button onClick$={loadMedia} class="btn btn-sm btn-outline mt-2">
                    Refresh Library
                  </button>
                </div>
              ) : (
                <div class={viewMode.value === 'grid' ? 'grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4' : 'space-y-2'}>
                  {filteredMedia.map((media) => {
                    const isSelected = mediaStore.selectedItems.some(item => item.id === media.id);
                        ...
                        {viewMode.value === 'grid' ? (
                          ...
                          <>
                            ...
                            <div class="flex-1 min-w-0">
                              <p class="text-sm font-medium text-gray-900 truncate">{media.filename}</p>
                              <p class="text-xs text-gray-500">{media.type} • {new Date(media.uploaded).toLocaleDateString()}</p>
                              {media.meta?.description && (
                                <p class="text-xs text-gray-400 truncate">{media.meta.description}</p>
                              )}
                            </div>
                            {isSelected && (
            ...
            <div class="flex items-center justify-between p-4 border-t bg-gray-50">
              <div class="text-sm text-gray-600">
                {mediaStore.selectedItems.length > 0 && (
                  <span>{mediaStore.selectedItems.length} item{mediaStore.selectedItems.length !== 1 ? 's' : ''} selected</span>
                )}
              </div>
              <div class="flex gap-2">
                <button
