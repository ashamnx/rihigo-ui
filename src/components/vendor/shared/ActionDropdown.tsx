...
export const ActionDropdown = component$<ActionDropdownProps>(({
    actions,
    align = 'end',
    size = 'sm',
}) => {
    return (
        <details class={`dropdown ${align === 'end' ? 'dropdown-end' : 'dropdown-start'}`}>
            <summary class={`btn btn-ghost btn-${size} btn-square`}>
                <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 12.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 18.75a.75.75 0 110-1.5.75.75 0 010 1.5z" />
                </svg>
            </summary>
            <ul class="dropdown-content menu p-2 shadow-lg bg-base-100 rounded-box w-52 border border-base-200 z-[999]">
                {actions.map((action, index) => (
                    <>
                        {action.dividerBefore && index > 0 && (
                            <li key={`divider-${index}`} class="my-1">
                        ...
                        <li key={index}>
                            <button
                                ...
                                class={[
                                    ...
                                    action.danger ? 'text-error hover:bg-error/10' : '',
                                    action.disabled ? 'opacity-50 cursor-not-allowed' : '',
                                ].filter(Boolean).join(' ')}
                                onClick$={() => {
                                    if (!action.disabled) {
                                        // Close the details dropdown
                                        const details = (document.activeElement as HTMLElement)?.closest('details');
