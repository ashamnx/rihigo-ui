...
export default component$(() => {
  ...
  const totalUsers = usersResponse.value.pagination_data?.total_count || 0;
  ...
  return (
    <div>
      ...
      {/* Filters */}
      <div class="bg-base-100 mb-6 rounded-lg p-4 shadow">
      ...
      {updateRoleAction.value?.success === false && (
        <div class="alert alert-error mb-4">
          ...
          <span>
            {updateRoleAction.value.error_message ||
      ...
      {deleteAction.value?.success === false && (
        <div class="alert alert-error mb-4">
          ...
          <span>
            {deleteAction.value.error_message || "Failed to delete user"}
          </span>
        </div>
      )}
      ...
      {filteredUsers.length === 0 ? (
        <div class="bg-base-100 rounded-lg p-12 text-center shadow">
          <svg
              ...
              {filteredUsers.map((user) => {
                ...
                return (
                  <tr key={user.id}>
                    ...
                    <td>
                      <span
                        class={`badge badge-sm ${getRoleBadgeColor(user.role)}`}
                      >
                        {user.role}
                      </span>
                    </td>
      ...
      {showRoleModal.value && selectedUser.value && (
        <div class="modal modal-open">
          <div class="modal-box">
            <h3 class="mb-4 text-lg font-bold">Update User Role</h3>

            <div class="mb-4">
              <p class="text-base-content/70 mb-2 text-sm">
                User:{" "}
                <span class="font-semibold">{selectedUser.value.name}</span>
              </p>
            ...
            <Form action={updateRoleAction} onSubmitCompleted$={closeRoleModal}>
              ...
              <div class="form-control mb-6">
                ...
                <select name="role" class="select select-bordered" required>
                  <option
                    value="user"
                    selected={selectedUser.value.role === "user"}
                  >
                    User
                  </option>
                  <option
                    value="admin"
                    selected={selectedUser.value.role === "admin"}
                  >
                    Admin
                  </option>
                </select>
      ...
      {showDeleteModal.value && selectedUser.value && (
        <div class="modal modal-open">
          <div class="modal-box">
            <h3 class="mb-4 text-lg font-bold">Delete User</h3>
            ...
            <div class="alert alert-warning mb-4">
              ...
              <span>
                This action cannot be undone. All user data and bookings will be
                affected.
              </span>
            </div>
