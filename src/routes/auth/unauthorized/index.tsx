          ...
          {session.value?.user ? (
            <>
              ...
              <Form action={signOut} class="w-full">
                ...
                <button
                  type="submit"
                  class="w-full inline-flex justify-center items-center gap-2 py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  disabled={signOut.isRunning}
                >
                  {signOut.isRunning && <span class="loading loading-spinner loading-xs"></span>}
                  {signOut.isRunning ? 'Signing out...' : (t('auth.unauthorized.signOut') || 'Sign Out')}
                </button>
              </Form>
            </>
          ) : (
